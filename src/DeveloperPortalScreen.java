...
public class DeveloperPortalScreen {
    ...
    private static VBox buildFormSection(Runnable onBack) {
        ...
        submitBtn.setOnAction(e -> {
            ...
            String desc   = descArea.getText().trim().replace("\"", "\\\""); // escape quotes
            ...
            // Build JSON payload using the global username
            String json = String.format(
                "{\"owner_username\":\"%s\",\"name\":\"%s\",\"description\":\"%s\",\"jar_url\":\"%s\",\"is_visible\":%d}",
                LauncherEngine.currentUsername, name, desc, jar, isVisible ? 1 : 0
            );

            ApiClient.postJson("submit_server.php", json).thenAccept(response -> {
