...
public class LauncherEngine {
    ...
    private static final String HUB_DIRECTORY = System.getProperty("user.home") + "/.rsps_hub/";
    ...
    public static void init() {
        try {
            Files.createDirectories(Paths.get(HUB_DIRECTORY));
            System.out.println("Hub initialized at: " + HUB_DIRECTORY);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static boolean downloadClient(ServerProfile server) {
        try {
            Path serverFolder = Paths.get(HUB_DIRECTORY, server.name.replaceAll(" ", "_"));
    ...
    public static boolean isDownloaded(ServerProfile server) {
        Path jarPath = Paths.get(HUB_DIRECTORY, server.name.replaceAll(" ", "_"), "SlothLite.jar");
